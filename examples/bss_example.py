...
if __name__ == "__main__":
    ...
    room = pra.ShoeBox(
        room_dim, fs=16000, max_order=15, absorption=0.35, sigma2_awgn=1e-8
    )
    ...
    if bss_type == "auxiva":
        ...
        # Run ILRMA
        Y = pra.bss.ilrma(
            X, n_iter=30, n_components=2, proj_back=True, callback=convergence_callback
        )
    elif bss_type == "fastmnmf":
        # Run FastMNMF
        Y = pra.bss.fastmnmf(
            X, n_iter=30, n_components=2, n_src=2, callback=convergence_callback
        )
        ...
        # Run SparseAuxIva
        Y = pra.bss.sparseauxiva(
            X, S, n_iter=30, proj_back=True, callback=convergence_callback
        )
