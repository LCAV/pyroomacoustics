...
wavfile.write(
    os.path.join(
        os.path.dirname(__file__), "output_samples", "denoise_input_Subspace.wav"
    ),
    fs,
...
# save to output file
enhanced_signal_fp = os.path.join(
    os.path.dirname(__file__), "output_samples", "denoise_output_Subspace.wav"
)
wavfile.write(enhanced_signal_fp, fs, pra.normalize(processed_audio).astype(np.float32))
