...
a1_b1=np.array([[0,0],[0,0],[pi/4,pi/4]])
a1_b2=np.array([[3*pi/4,3*pi/4],[3*pi/4,pi/2],[pi/2,pi/2]])
a2_b1=np.array([[0,0]])
    ...
    if(type_coordinates=="3-D"):
        ...
        for a in a_range:
            for b in b_range:
                ...
                error_elevation=(angle_function(a,b)- a_b_range[a_b_index])[:,1]

                print("for points :")
                print(a,b)
                print("error in azimuth calculation: {}".format(np.average(error_azimuth)))
                print("error in elevation calculation: {}".format(np.average(error_elevation)))
                print()
                a_b_index+=1
        ...
        for c in c_range:
            for d in d_range:
                ...
                error_elevation=(angle_function(c,d) - c_d_range[c_d_index])[:,1]
