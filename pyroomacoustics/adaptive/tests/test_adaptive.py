...
class TestAdaptiveFilter(TestCase):
    def test_rls(self):
        rls = pra.adaptive.RLS(length, lmbd=1.0, delta=2.0)
    ...
    def test_block_rls(self):
        block_rls = pra.adaptive.BlockRLS(length, lmbd=1.0, delta=2.0)
    ...
    def test_block_lms(self):
        block_lms = pra.adaptive.BlockLMS(length, mu=1.0 / length / 2.0)
