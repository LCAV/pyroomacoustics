...
class DIRPATInterpolate():
    ...
    def __init__(self,path,fs,directivity_pattern=2,source=False,interpolate=True,no_of_points_fibo_sphere=1200,azimuth_simulation=0,colatitude_simulation=0):
    ...
    def plot(self,freq_bin=0):
        ...
        if self.interpolate:
            ...
            ax_2=ax3.scatter(self.rotated_fibo_points[0,:], self.rotated_fibo_points[1,:], self.rotated_fibo_points[2,:], c=np.abs(fft(self.sh_coeffs_expanded_target_grid,axis=-1)[:,freq_bin]),s=50)
            ...
            ax_1 = ax2.scatter(self.rotated_sofa_points[0,:], self.rotated_sofa_points[1,:], self.rotated_sofa_points[2,:], c=np.abs(self.freq_angles_fft[:, freq_bin]), s=100)
            ax2.set_title("Expanded Initial Grid")
            fg.colorbar(ax_1, shrink=0.5, aspect=5)
