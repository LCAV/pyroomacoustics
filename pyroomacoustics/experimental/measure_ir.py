...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="measure_ir",
        description="Measures an impulse response by playing a sweep and recording it using the sounddevice package.",
    )
    parser.add_argument(
        "-l", "--length", type=float, default=1.0, help="length of the sweep in seconds"
    )
    parser.add_argument(
        "-t",
        "--type",
    ...
    parser.add_argument(
        "--f-lo", type=float, default=0.0, help="lowest frequency in the sweep"
    )
    parser.add_argument(
        "--f-hi",
        type=float,
    ...
    parser.add_argument("--dev-in", type=int, default=None, help="input device number")
    parser.add_argument(
        "--dev-out", type=int, default=None, help="output device number"
    )
    parser.add_argument(
        "--ch-in",
        dest="channels_input_mapping",
        action="append",
        type=int,
    ...
    parser.add_argument(
        "-p", "--plot", action="store_true", help="plot the resulting signal"
    )
