...
def measure_rt60(h, fs=1, decay_db=60, plot=False, rt60_tgt=None):
    ...
    if plot:
        ...
        # time vector
        get_time = lambda x, fs: np.arange(x.shape[0]) / fs
        ...
        # now the linear fit
        decay_rate = (5 + decay_db) / decay_time
        plt.plot([t_5db, t_decay], [e_5db, e_decay], '--', label='Linear Fit')
        plt.plot(T, np.ones_like(T) * -60, '--', label='-60 dB')
        plt.vlines(est_rt60, energy_db_min, 0, linestyles='dashed', label='Estimated RT60')
